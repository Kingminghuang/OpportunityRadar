...
4. 只生成符合 `docs/content-schema.md` 的 Opportunity Radar JSON。路径固定为 `_data/reports/YYYY-MM-DD.json`，文件名日期使用 `Asia/Singapore` 当地日期。文件内容必须是有效 UTF-8 JSON；不要使用 Markdown 代码围栏，也不要附加 JSON 以外的说明。
...
7. 通过 GitHub 连接器检查 `Kingminghuang/OpportunityRadar` 中该 JSON 路径是否存在：不存在则 create；存在则读取当前 blob SHA 后 update。不要重复创建同一天的多个文件。
8. 同步完成后，在最终通知中给出：今日 Top 1、写入的 JSON 仓库路径、本次是 create 还是 update，以及对应提交的 GitHub Actions 发布状态链接（如可取得）。Pages 构建成功后，补充网页链接 `https://kingminghuang.github.io/OpportunityRadar/reports/YYYY-MM-DD/` 和 Markdown 归档链接 `https://kingminghuang.github.io/OpportunityRadar/archives/YYYY-MM-DD.md`。若 GitHub 写入失败，明确报告失败原因，不要声称已同步。
