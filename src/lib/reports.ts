...
export const TRACKS = ['developer', 'office'] as const;
...
const reportSchema = z
  .object({
    ...
    track: z.enum(TRACKS).default('developer'),
    title: z.string().trim().min(1),
    summary: z.string().trim().min(1),
...
export function trackLabel(track: Track): string {
  return { developer: '开发者', office: '白领办公' }[track];
}

export function trackFromPath(path: string): Track {
  return path.includes('_data/office-reports/') ? 'office' : 'developer';
}
...
export function reportHref(base: string, reportSlugValue: string, opportunitySlug?: string, track?: Track): string {
  const prefix = track === 'office' ? 'office/' : '';
  const reportPath = sitePath(base, `${prefix}reports/${reportSlugValue}/`);
...
export function parseReports(rawReports: RawReport[]): Report[] {
  return rawReports
    .map(({ path, data }) => {
      ...
      const filenameDate = path.match(/(?:reports|office-reports)\/(\d{4}-\d{2}-\d{2})\.json$/)?.[1];

      if (filenameDate !== parsed.date) {
        throw new Error(`报告文件名日期必须与 JSON date 一致：${path}`);
      }

      const expectedTrack = trackFromPath(path);
      if (parsed.track !== expectedTrack) {
        throw new Error(`报告 track 与所在目录不一致：${path}（track=${parsed.track}，目录=${expectedTrack}）`);
      }
